...
@Composable
fun ArchivedChatsLazyColumn(itemsList: MutableState<List<Chat>>, callback: (Chat) -> Unit, listState: LazyListState) {
    val searchText = rememberSaveable { mutableStateOf("") }
...
@Composable
fun ContactsLazyColumn(itemsList: List<Chat>, callback: (Chat) -> Unit, listState: LazyListState) {
    LazyColumn(
        state = listState,
        modifier = Modifier
            .fillMaxSize() // 确保 LazyColumn 填满父容器
