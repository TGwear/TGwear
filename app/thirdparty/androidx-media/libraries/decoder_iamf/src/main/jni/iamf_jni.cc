...
IAMF_decoder_peak_limiter_enable(decoder,
0);
...
env->
ReleaseByteArrayElements(initializationDataArray,
...
reinterpret_cast
<IAMF_DecoderHandle>(decoderRawPointer),
reinterpret_cast
<const uint8_t *>(
        env
->
GetDirectBufferAddress(inputBuffer)
),
inputSize, rsize,
reinterpret_cast
<void *>(env
->
GetDirectBufferAddress(outputBuffer)
));
}
...
return
IAMF_layout_sound_system_channels_count((IAMF_SoundSystem)
soundSystem);
}

DECODER_FUNC(void, iamfClose, jlong decoderRawPointer) {
IAMF_decoder_close(reinterpret_cast
<IAMF_DecoderHandle>(decoderRawPointer)
);
}
