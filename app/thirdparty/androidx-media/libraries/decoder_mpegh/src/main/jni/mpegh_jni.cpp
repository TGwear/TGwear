...
env->
ThrowNew(atscExCls,
"not possible to get direct byte buffer!");
return 0;
}
...
ctx->
outSampleRate = outInfo.sampleRate;
ctx->
outNumChannels = outInfo.numChannels;
ctx->
outPts = outInfo.pts / 1000;
} else {
ctx->
outSampleRate = -1;
ctx->
...
return
outNumSamples *ctx
->outNumChannels * BYTES_PER_SAMPLE;
}
