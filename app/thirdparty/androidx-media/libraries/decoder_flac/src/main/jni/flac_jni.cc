...
for (
std::vector<std::string>::const_iterator vorbisComment =
...
env->
CallBooleanMethod(commentList, arrayListAddMethod, commentString
);
env->
DeleteLocalRef(commentString);
}
}
...
for (
std::vector<FlacPicture>::const_iterator picture = pictures.begin();
picture != pictures.
...
env->
SetByteArrayRegion(pictureData,
0, picture->data.
...
env->
CallBooleanMethod(pictureFrames, arrayListAddMethod, pictureFrame
);
env->
DeleteLocalRef(mimeType);
env->
DeleteLocalRef(description);
env->
DeleteLocalRef(pictureData);
}
}
...
return env->
NewObject(flacStreamMetadataClass, flacStreamMetadataConstructor,
        streamInfo
.min_blocksize, streamInfo.max_blocksize,
...
readBuffer(outputBuffer, outputSize
);
}
...
env->
ReleaseByteArrayElements(jOutputArray, outputBuffer,
0);
return
count;
}
...
if (success) {
env->
SetLongArrayRegion(outSeekPoints,
0, result.
